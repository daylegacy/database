...
public:
	...
	void delete_tree_r(nodeT * root) {
		if (!root) return;
		delete_tree_r(root->link[0]);
		delete_tree_r(root->link[1]);
		delete root;
	}
	void delete_tree() {
		delete_tree_r(r_root());
		pre_root->link[0] = nullptr;
		avl_size = 0;
	}
	void reset_tree() {
		pre_root->link[0] = nullptr;
	...
	inline nodeT * find(const void * item, long long (record::*compare)(const record&)const) {
		int cmp;
		auto p = pre_root;
	...
	void print_no_fr(nodeT *node) {
		if (!node) return;
		node->pdata->print();
		print_no_fr(node->link[0]);
		print_no_fr(node->link[1]);
	}
	...
	inline const T * avl_delete(const T * item, long long (record::*compare)(const record&)const)
	{
	...
	inline nodeT * avl_insert(T * item, long long (record::*compare)(const record&)const)
	{
	...
	bool is_it_avl(long long (record::*compare)(const record&)const) {
	...
	int go_around_check(nodeT * root, long long (record::*compare)(const record&)const) {
		if (!root) return 0;
